      ...
      <main class="dashboard-content">
        <div class="container-fluid px-3 px-lg-4 py-4">
          ...
          <section class="panel">
            ...
            <div class="table-responsive"><table class="table align-middle mb-0" id="ordersTable" data-searchable-table><thead><tr><th>ID</th><th>Name</th><th>Gender</th><th>Phone</th><th>Email</th><th>District</th><th class="text-end">Action</th></tr></thead><tbody>

              @foreach ($students as $student )
                
              
              <tr><td class="fw-semibold">{{$student->id}}</td><td><div class="table-media"><img class="product-thumb" src="../assets/images/ecommerce/product-1.jpg" alt="Wireless Headset"><span>{{$student->name}}</span></div></td><td>{{$student->gender}}</td><td><span class="badge text-bg-success">{{$student->phone}}</span></td><td>{{$student->email}}</td><td>{{$student->district}}</td><td class="text-end"><button class="btn btn-light btn-sm" type="button">View</button></td></tr>
              <tr></tr>
              @endforeach
            </tbody></table></div>
          </section>
